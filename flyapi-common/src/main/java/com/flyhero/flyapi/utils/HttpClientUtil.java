...
/*		JSONObject jsonpost = HttpClientUtil.getUrl("post",
	...
	public static JSONObject getUrl(String method, StringBuffer requestUrl,
...
//				requestUrl.append("?");
			...
			/*	String s=params.replace(" ", "").replace("\"", "").replace("{", "").replace("}", "").replace(":", "=").replace(",", "&").replace("\n", "").replace("\t", "");
				requestUrl.append(s);
				System.out.println(s);
				System.out.println(requestUrl);
				System.out.println(s);
				jsonobject = sendPostRequest(requestUrl, "");*/
			}else{
			...
			if(jsonWay == 0){
				...
				for (Map.Entry<String, Object> entry : json.entrySet()) {
					...
					requestUrl.append(entry.getKey() + "=" + param + "&");
				}
...
//				requestUrl=requestUrl.substring(0, index);
				...
				requestUrl.append("?jsonString="+s);
				System.out.println(requestUrl);
				jsonobject = sendGetRequest(requestUrl);
	...
	public static JSONObject sendGetRequest(StringBuffer requestUrl) {
		JSONObject json = new JSONObject();
		long responseLength = 0; // 响应长度
		String responseContent = null; // 响应内容
		HttpClient httpClient = new DefaultHttpClient(); // 创建默认的httpClient实例
		HttpGet httpGet = new HttpGet(requestUrl.toString()); // 创建HttpGet
		try {
	 ...
	 */
	@SuppressWarnings("finally")
	public static JSONObject sendPostRequest(StringBuffer requestUrl,
			List<NameValuePair> formParams) {
		JSONObject json = new JSONObject();
		long responseLength = 0; // 响应长度
		String responseContent = null; // 响应内容
		HttpClient httpClient = new DefaultHttpClient(); // 创建默认的httpClient实例
		HttpPost httpPost = new HttpPost(requestUrl.toString()); // 创建HttpPost
